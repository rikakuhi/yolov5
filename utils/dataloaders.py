...
class LoadImages:
    ...
    def __init__(self, path, img_size=640, stride=32, auto=True, transforms=None, vid_stride=1, crop=None):
    ...
    def __next__(self):
        ...
        if self.transforms:
            ...
            if self.crop:
                im = im0[self.crop[0][1]:self.crop[1][1], self.crop[0][0]:self.crop[1][0], :]
    ...
    def _new_video(self, path):
        # Create a new video capture object 用opencv，读取video文件
        self.frame = 0
        self.cap = cv2.VideoCapture(path)
        self.frames = int(self.cap.get(cv2.CAP_PROP_FRAME_COUNT) / self.vid_stride)  # 此段视频共提取出多少帧图片进行预测。
    ...
    def __init__(self,
        ...
        try:
            ...
            self.im_files = sorted(
                x.replace('/', os.sep) for x in f if x.split('.')[-1].lower() in IMG_FORMATS)  # 获取图片路径
            # self.img_files = sorted([x for x in f if x.suffix[1:].lower() in IMG_FORMATS])  # pathlib
        ...
        # Check cache
        self.label_files = img2label_paths(self.im_files)  # labels 路径
    ...
    def load_image(self, i):
        ...
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                ...
                im = cv2.imread(f)  # 读取该张图片，得到对应的ndarray
                assert im is not None, f'Image Not Found {f}'
            h0, w0 = im.shape[:2]  # 原始图片的h和w
...
def verify_image_label(args):
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            nf = 1  # label found
            with open(lb_file) as f:
                # lb = [x.split() for x in f.read().strip().splitlines() if len(x) if x[0] == "-"]
                lb = []
                    ...
                    if len(x):
                        ...
                        for index, _x in enumerate(x):
                            ...
                            if float(_x) > 1:
                                x[index] = "1"
                        lb.append(x)
                if any(len(x) > 6 for x in lb):  # is segment 判断是否需要分割
