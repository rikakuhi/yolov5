...
def segletterbox(im, label, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True,
                 stride=32):
...
def letterbox(im, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True, stride=32):
    ...
    new_unpad = int(round(shape[1] * r)), int(round(shape[0] * r))
    dw, dh = new_shape[1] - new_unpad[0], new_shape[0] - new_unpad[1]  # wh padding
    if auto:  # 这里其实就是将当前缩放完的边，用最少的像素填充为64或者是32的倍数，以利用有效的信息。
        dw, dh = np.mod(dw, stride), np.mod(dh, stride)  # 总共是1280，完全填充需要dh，除以64(32),取余，就是需要填充的
    elif scaleFill:  # stretch
        dw, dh = 0.0, 0.0
        new_unpad = (new_shape[1], new_shape[0])
        ratio = new_shape[1] / shape[1], new_shape[0] / shape[0]  # width, height ratios

    dw /= 2  # 将需要填充的，分为两份，在图片上、下或者左、右分别填充
    dh /= 2
