...
LOCAL_RANK = int(os.getenv('LOCAL_RANK', -1))  # https://pytorch.org/docs/stable/elastic/run.html  主进程设置为-1
...
# todo 看一下logg，以后可以模仿用这个类来记录和输出网络的一些信息
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    # Directories
    w = save_dir / 'weights'  # 保存权重文件夹
    (w.parent if evolve else w).mkdir(parents=True, exist_ok=True)  # 生成weights这个文件夹
    last, best = w / 'last.pt', w / 'best.pt'  # best.pt和last.pt的存放路径

    if isinstance(hyp, str):
        with open(hyp, errors='ignore') as f:
            hyp = yaml.safe_load(f)  # load hyp dict
    ...
    if not evolve:
        ...
        yaml_save(save_dir / 'opt.yaml', vars(opt))  # 保存训练时候，各种模式信息
    ...
    if pretrained:
        ...
        ckpt = torch.load(weights, map_location='cpu')  # 加载模型到cpu 避免显存爆炸。
    ...
    # Model attributes
    nl = de_parallel(model).model[-1].nl  # number of detection layers (to scale hyps) 最后有几个检测头
...
def parse_opt(known=False):
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/my_data_seg.yaml', help='dataset.yaml path')
    ...
    parser.add_argument("--only_seg", type=bool, default=False, help="Only segment in training")
...
def main(opt, callbacks=Callbacks()):
    ...
    # print_args(vars(opt))
    # check_git_status()
    # check_requirements()  # 检查当前环境和requirement.txt是否匹配

    # Resume
    if opt.resume and not opt.evolve:  # 是否接着训练
        ...
        opt.save_dir = str(increment_path(Path(opt.project) / opt.name, exist_ok=opt.exist_ok))  # 保存路径

    # DDP  多卡训练
    device = select_device(opt.device, batch_size=opt.batch_size)
    if LOCAL_RANK != -1:  # main里面必须是主进程
