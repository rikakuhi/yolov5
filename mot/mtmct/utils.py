...
def parse_pt(mot_feature, zones=None):
    """返回一个dict，key为跟踪目标的跟踪id，value仍然是字典，这个字典的key为第几帧图片，value为dict,包含bbox、跟踪id、img name、feature、zone、frame name"""
    mot_list = dict()
    for line in mot_feature:
        fid = int(re.sub('[a-z,A-Z]', "", mot_feature[line]['frame']))  # 第几帧图片
        tid = mot_feature[line]['id']  # 跟踪目标的id
        ...
        if zones is not None:
            ...
            out_dict['zone'] = None
        mot_list[tid][fid] = out_dict  # 先确定跟踪号，在确定是第几帧图片
    return mot_list
...
def gen_new_mot(mot_list):
    ...
    for tracklet in mot_list:
        tracklet = mot_list[tracklet]  # 一个dict，key为第几帧图片，value为对应的检测框信息
        for f in tracklet:
            out_dict[tracklet[f]['imgname']] = tracklet[f]
    return out_dict  # key为每一帧图片中的每一个实例名称(根据一定规则)，value
...
def mergesetfeat1_notrk(P, neg_vector, in_feats, in_labels):
    ...
    for i in range(in_feats.shape[0]):
        ...
        feat = in_feats[i] - neg_vector[camera_id]  # 减去均值
...
def compute_P2(prb_feats, gal_feats, gal_labels, la=3.0):
    X = gal_feats
    neg_vector = {}  # key:摄像头的id，value:该摄像头内，跟踪目标的feature的均值
    u_labels = np.unique(gal_labels[:, 1])
    P = {}
    for label in u_labels:
        curX = gal_feats[gal_labels[:, 1] == label, :]  # 取出第一个摄像头中的所有跟踪目标的feature
...
def intracam_ignore(st_mask, cid_tids):
    """制作一个矩阵，横纵坐标都是跟踪到的目标，如果同一个镜头，对应的值就是0，不同镜头对应的值是1"""
    count = len(cid_tids)
...
def batch_numpy_topk(qf, gf, k1, N=6000):
    ...
    for j in range(n // N + 1):
        ...
        temp_qd = temp_qd.T
        initial_rank.append(find_topk(temp_qd, k=k1, axis=1, largest=False, sorted=True)[1])
    del temp_qd
    del temp_gf
    del temp_qf
    del temp_d
...
def ReRank2(probFea, galFea, k1=20, k2=6, lambda_value=0.3):
    ...
    initial_rank = batch_numpy_topk(feat, feat, k1 + 1, N=6000)  # 找到最相近的前20个？
    del probFea
    del galFea
...
def visual_rerank(prb_feats, gal_feats, cid_tids, use_ff=False, use_rerank=False):
    """Rerank by visual cures. 求出一个相似矩阵，通过一系列方法"""
    ...
    if use_ff:
        ...
        prb_feats, gal_feats = run_fic(prb_feats, gal_feats, prb_labels, gal_labels, 3.0)  # 这里像是进行了类似norm之类的操作。

        # Step1=2: fac. finetuned parameters: [beta,knn,lr,prb_epoch,gal_epoch]
        prb_feats, gal_feats = run_fac(prb_feats, gal_feats, prb_labels, gal_labels, 0.08, 20, 0.5, 1, 1)  # 同上

    if use_rerank:
        print('current use rerank finetuned parameters....')
        # Step2: k-reciprocal. finetuned parameters: [k1,k2,lambda_value]  k近邻算法
...
def get_labels(cid_tid_dict, cid_tids, use_ff=True, use_rerank=True, use_st_filter=False):
    # 1st cluster  求出两个镜头中跟踪目标的相似度，数值越大，相似度越大(实际是求二者之间的距离，求完距离之后用1减去距离，得到的就是相似度)
    ...
    cluster_labels = AgglomerativeClustering(  # 层次聚类，将得到的相似度矩阵，扔到里面，求出聚类后的label
        n_clusters=None,
        distance_threshold=0.5,
        affinity='precomputed',
        linkage='complete').fit_predict(1 - sim_matrix)
    labels = get_match(cluster_labels)  # 得到匹配结果
    sub_cluster = get_cid_tid(labels, cid_tids)  # 在id匹配的基础上，添加上相机信息。

    # 2nd cluster，再一次聚类，方法和第一次相同
    cid_tid_dict_new = combin_feature(cid_tid_dict, sub_cluster)  # 对第一次聚类匹配上的id，更新他们的mean feature
...
def getData(fpath, names=None, sep='\s+|\t+|,'):
    ...
    """
    try:
        df = pd.read_csv(fpath, sep=sep, index_col=None, skipinitialspace=True, header=None, names=names,
                         engine='python')
        return df
