...
class DeepSORTTracker(object):
    ...
    def update(self, pred_dets, pred_embs):
        ...
        detections = [Detection(tlwh, score, feat, cls_id)
                      for tlwh, score, feat, cls_id in zip(pred_tlwhs, pred_scores, pred_embs, pred_cls_ids)
                      ]  # 针对每个detection构建一个Detection类用来存放相关信息

        # Run matching cascade.
        matches, unmatched_tracks, unmatched_detections = self._match(detections)

        # 针对匹配结果的不同，分别进行update
        for track_idx, detection_idx in matches:
            self.tracks[track_idx].update(self.motion, detections[detection_idx])
        for track_idx in unmatched_tracks:
            self.tracks[track_idx].mark_missed()
        for detection_idx in unmatched_detections:  # 未匹配上的detection，就初始化为track(unconfirmed)
    ...
    def _match(self, detections):
        def gated_metric(tracks, dets, track_indices, detection_indices):
            ...
            cost_matrix = gate_cost_matrix(self.motion, cost_matrix, tracks, dets, track_indices, detection_indices)
            return cost_matrix
